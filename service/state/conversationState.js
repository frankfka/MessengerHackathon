...
function getNewState(currentState, action) {
  const newState = { ...currentState }; // State shouldn't have nested properties
  const currentStateCode = currentState[STATE_CURR_STATE_KEY];
  const actionCode = action[ACTION_CODE_KEY];
